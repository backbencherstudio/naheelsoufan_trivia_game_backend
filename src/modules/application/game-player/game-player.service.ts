...
export class GamePlayerService {
    ...
    async nextTurn(gameId: string) {
        try {
            ...
            let nextPlayerIndex;
            if (currentPlayerIndex === -1 || currentPlayerIndex === game.game_players.length - 1) {
    ...
    async updateGamePhase(gameId: string, phase: string) {
        try {
            const validPhases = ['waiting', 'category_selection', 'question', 'completed'];
    ...
    async addGuestPlayer(gameId: string, playerName: string) {
        try {
            ...
            if (existingPlayer) {
                throw new BadRequestException('Player name already exists in this game');
            }
    ...
    async validateGuestPlayerName(playerName: string) {
        try {
            ...
            if (!validNameRegex.test(trimmedName)) {
                return { isValid: false, error: 'Player name can only contain letters, numbers, spaces, hyphens, and underscores' };
            }

            // Check for inappropriate words (basic check)
            const inappropriateWords = ['admin', 'moderator', 'system', 'bot', 'guest'];
            const lowerName = trimmedName.toLowerCase();
            for (const word of inappropriateWords) {
                if (lowerName.includes(word)) {
                    return { isValid: false, error: 'Player name contains inappropriate words' };
                }
            }
     ...
     */
    async selectQuickGameCategory(gameId: string, categoryId: string, difficultyId: string) {
        try {
            const game = await this.prisma.game.findUnique({
                where: { id: gameId },
                include: {
    ...
    async getQuickGameQuestion(gameId: string) {
        try {
            ...
            if (!gameSelection) {
                throw new BadRequestException('No category/difficulty selected for current turn');
            }
            ...
            if (questions.length === 0) {
                throw new NotFoundException('No questions available for selected category and difficulty');
            }
     ...
     */
    async answerQuickGameQuestion(gameId: string, questionId: string, answerId: string) {
        try {
            const game = await this.prisma.game.findUnique({
                where: { id: gameId },
                include: {
     ...
     */
    async stealQuickGameQuestion(gameId: string, questionId: string, answerId: string, userId: string) {
        try {
            const game = await this.prisma.game.findUnique({
                where: { id: gameId },
                include: {
            ...
            const stealingPlayer = game.game_players.find(p => p.user_id === userId);
            if (!stealingPlayer) {
                const availableUserIds = game.game_players.map(p => p.user_id).filter(id => id !== null);
     ...
     */
    async addMultipleQuickGamePlayers(gameId: string, playerNames: string[]) {
        try {
            ...
            const maxPlayers = 4; // Quick Game max
            const totalPlayersAfterAdding = game._count.game_players + playerNames.length;
            ...
            if (invalidNames.length > 0) {
                throw new BadRequestException(`Invalid player names: ${invalidNames.map(r => r.error).join(', ')}`);
            }
            ...
            if (existingPlayers.length > 0) {
                const existingNames = existingPlayers.map(p => p.player_name).join(', ');
                throw new BadRequestException(`Player names already exist in this game: ${existingNames}`);
            }
            ...
            const createdPlayers = await this.prisma.$transaction(async (tx) => {
                ...
                for (const playerName of playerNames) {
                    const validationResult = validationResults.find(r => r.sanitizedName === playerName.trim());
                    const sanitizedName = validationResult?.sanitizedName || playerName.trim();
            ...
            return {
                ...
                data: {
                    ...
                    remaining_slots: maxPlayers - (game._count.game_players + createdPlayers.length)
                }
            };
    ...
    async addPlayersAndStartQuickGame(gameId: string, playerNames: string[]) {
        try {
            ...
            const addResult = await this.addMultipleQuickGamePlayers(gameId, playerNames);
     ...
     */
    async selectCompetitiveCategory(gameId: string, categoryId: string, difficultyId: string) {
        try {
            const game = await this.prisma.game.findUnique({
                where: { id: gameId },
                include: {
     ...
     */
    async answerCompetitiveQuestion(gameId: string, questionId: string, answerId: string, playerId: string) {
        try {
            const game = await this.prisma.game.findUnique({
                where: { id: gameId },
                include: {
