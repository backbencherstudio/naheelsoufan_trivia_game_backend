...
export class SubscriptionService {
    ...
    async getUserSubscriptions(user_id: string) {
        ...
        const data = subscriptions.map(sub => ({
            ...
            payment_provider_charge: sub.payment_provider_charge ? Number(sub.payment_provider_charge) : undefined,
    ...
    async getSubscriptionStatus(user_id: string) {
        ...
        const data = {
            ...
            can_play_games: gamesRemaining > 0 || activeSubscription.subscription_type.games === -1, // -1 means unlimited
            games_remaining: activeSubscription.subscription_type.games === -1 ? undefined : gamesRemaining,
            subscription_type: {
    ...
    async purchaseSubscription(user_id: string, dto: PurchaseSubscriptionDto) {
        ...
        if (existingSubscription) {
            throw new ConflictException('User already has an active subscription of this type');
        }
        ...
        try {
            ...
            console.error('Error creating Stripe payment intent for subscription:', error);
    ...
    async cancelSubscription(user_id: string, dto: CancelSubscriptionDto) {
        ...
        const data = {
            ...
            paid_amount: updatedSubscription.paid_amount ? Number(updatedSubscription.paid_amount) : undefined,
            ...
            payment_provider_charge_type: updatedSubscription.payment_provider_charge_type,
            payment_provider_charge: updatedSubscription.payment_provider_charge ? Number(updatedSubscription.payment_provider_charge) : undefined,
    ...
    async handlePaymentSuccess(paymentIntentId: string) {
        ...
        if (!subscription) {
            console.error('Subscription not found for payment intent:', paymentIntentId);
            return;
        }
    ...
    async canUserPlayGame(user_id: string, game_mode: string) {
        ...
        const gamesRemaining = activeSubscription.subscription_type.games - activeSubscription.games_played_count;
        const canPlay = gamesRemaining > 0 || activeSubscription.subscription_type.games === -1; // -1 means unlimited

        return {
            success: true,
            message: canPlay ? 'User can play this game mode' : 'User cannot play this game mode',
            data: {
                can_play: canPlay,
                game_mode,
                requires_subscription: true,
    ...
    async getSubscriptionById(user_id: string, subscription_id: string) {
        ...
        const data = {
            ...
            paid_amount: subscription.paid_amount ? Number(subscription.paid_amount) : undefined,
            ...
            payment_provider_charge: subscription.payment_provider_charge ? Number(subscription.payment_provider_charge) : undefined,
